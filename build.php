		...
		function handleMarketplace() {
					...
					if ($this->merchantProperty['confirm_snd']) {
						...
						if (( ( ( isset( $_POST['x'] ) && isset( $_POST['y'] ) ) && trim( $_POST['x'] ) != '' ) && trim( $_POST['y'] ) != '' )) {
							...
							if (( isset( $_POST['vname'] ) && trim( $_POST['vname'] ) != '' )) {
								$vRow = $this->m->getVillageDataByName( trim( $_POST['vname'] ) );
							} else {
								if (isset( $_GET['vid2'] )) {
									$vRow = $this->m->getVillageDataById( intval( $_GET['vid2'] ) );
						...
						$doSend = TRUE;
						$vRow = $this->m->getVillageDataById( intval( $_POST['vid2'] ) );
			...
			if ($this->selectedTabIndex == 1) {
				...
				if (( isset( $_GET['oid'] ) && 0 < intval( $_GET['oid'] ) )) {
					$oRow = $this->m->getOffer2( intval( $_GET['oid'] ), $this->data['rel_x'], $this->data['rel_y'], $this->setupMetadata['map_size'] / 2 );
			...
			if ($this->selectedTabIndex == 2) {
				...
				if ($this->isPost()) {
					if (( ( ( ( ( ( ( isset( $_POST['m1'] ) && 0 < intval( $_POST['m1'] ) ) && isset( $_POST['m2'] ) ) && 0 < intval( $_POST['m2'] ) ) && isset( $_POST['rid1'] ) ) && 0 < intval( $_POST['rid1'] ) ) && isset( $_POST['rid2'] ) ) && 0 < intval( $_POST['rid2'] ) )) {
						...
						if (( $this->merchantProperty['available_res'] && !$this->merchantProperty['showError'] )) {
							...
							if (( 0 < $this->merchantProperty['vRow_merchant_num'] && $this->merchantProperty['vRow_merchant_num'] <= $this->merchantProperty['exits_num'] )) {
								...
								$this->m->addMerchantOffer( $this->player->playerId, $this->data['name'], $this->data['selected_village_id'], $this->data['rel_x'], $this->data['rel_y'], $this->merchantProperty['vRow_merchant_num'], $offer, isset( $_POST['ally'] ), (( ( isset( $_POST['d1'] ) && isset( $_POST['d2'] ) ) && 0 < intval( $_POST['d2'] ) ) ? intval( $_POST['d2'] ) : 0), $this->gameMetadata['tribes'][$this->data['tribe_id']]['merchants_velocity'] * $this->gameMetadata['game_speed'] );
					...
					if (( isset( $_GET['d'] ) && 0 < intval( $_GET['d'] ) )) {
						$row = $this->m->getOffer( intval( $_GET['d'] ), $this->player->playerId, $this->data['selected_village_id'] );
						...
						if ($row != NULL) {
							...
							$this->queueModel->_updateVillage( FALSE, FALSE );
							$this->m->removeMerchantOffer( intval( $_GET['d'] ), $this->player->playerId, $this->data['selected_village_id'] );
						}
					}
				}
		...
		function handleWarrior() {
			...
			if (( ( ( $this->isPost() && isset( $_POST['tf'] ) ) && !$this->isGameTransientStopped() ) && !$this->isGameOver() )) {
				...
				if ($canProcess) {
					...
					$this->m->decreaseGoldNum( $this->player->playerId, $totalGoldsNeeded );
					$this->m->dispose();
